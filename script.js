...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.getElementById('adminHeroForm').addEventListener('submit', (e) => {
    ...
    DataManager.set(data).then(() => {
      renderAdminDashboard();
      renderAll();
      notifySaveResult(true, 'Hero section');
    }).catch(err => {
  ...
  document.getElementById('adminExpForm').addEventListener('submit', (e) => {
    ...
    promise.then(() => {
      ...
      renderAdminDashboard();
      renderAll();
      notifySaveResult(true, 'Experience entry');
    }).catch(() => {
  ...
  document.getElementById('adminEduForm').addEventListener('submit', (e) => {
    ...
    promise.then(() => {
      ...
      renderAdminDashboard();
      renderAll();
      notifySaveResult(true, 'Education entry');
    }).catch(() => {
  ...
  document.getElementById('adminAboutForm').addEventListener('submit', (e) => {
    ...
    DataManager.update('about.text', document.getElementById('aboutTextarea').value)
      .then(() => {
        renderAdminDashboard();
        renderAll();
        notifySaveResult(true, 'About text');
      }).catch(() => {
  ...
  document.getElementById('adminCertForm').addEventListener('submit', (e) => {
    ...
    promise.then(() => {
      ...
      renderAdminDashboard();
      renderAll();
      notifySaveResult(true, 'Certificate');
    }).catch(() => {
  ...
  document.getElementById('confirmAddCategoryBtn').addEventListener('click', () => {
    ...
    DataManager.addCategory(name).then(() => {
      populateCategorySelect();
      document.getElementById('projectCategory').value = name;
      renderProjectFilters();
      document.getElementById('inlineAddCategoryRow').style.display = 'none';
      input.value = '';
      notifySaveResult(true, 'Category');
    }).catch(() => {
  ...
  document.getElementById('adminTimelineForm').addEventListener('submit', (e) => {
    ...
    promise.then(() => {
      ...
      renderAdminDashboard();
      renderAll();
      notifySaveResult(true, 'Timeline item');
    }).catch(() => {
  ...
  document.getElementById('adminSocialForm').addEventListener('submit', (e) => {
    ...
    promise.then(() => {
      ...
      renderAdminDashboard();
      renderAll();
      notifySaveResult(true, 'Social link');
    }).catch(() => {
  ...
  document.getElementById('adminContactForm').addEventListener('submit', (e) => {
    ...
    DataManager.update('contact.text', document.getElementById('contactTextarea').value)
      .then(() => {
        renderAdminDashboard();
        renderAll();
        notifySaveResult(true, 'Contact text');
      }).catch(() => {
  ...
  document.getElementById('adminResetDataBtn').addEventListener('click', () => {
    ...
    DataManager.reset().then(() => {
      renderAdminDashboard();
      renderAll();
      notifySaveResult(true, 'Reset');
    }).catch(() => {
 ...
 * Rashed's Portfolio – Full Admin + Public Logic with Undo
 * Updated: hero current role, about cards show skill names, timeline reversed,
 * skills redesigned with filter & cards, projects no click, pie tooltip fixed.
 * Fixed: file upload for profile image, all admin save operations.
 */

// ============================================================
// DATA LAYER – encrypted localStorage
// ============================================================
...
// Password hashing (for admin login gate only — NOT used to encrypt content)
function hashPassword(pw) {
  return CryptoJS.SHA256(pw).toString();
}
...
const DataManager = {
  ...
  // Loads portfolio content from localStorage as plain JSON.
  // IMPORTANT: content is intentionally NOT encrypted — this is public
  // portfolio data that must render for every visitor without a password.
  // Only the admin password itself is protected (as a SHA-256 hash, see AUTH below).
  init() {
    const stored = localStorage.getItem('rashed_portfolio_data');
    if (stored) {
      try {
        ...
        console.error('Saved portfolio data was corrupted and could not be read:', err);
      }
      // Corrupted data: keep the broken string in a backup key so nothing is
      // silently destroyed, then fall back to defaults.
      try { localStorage.setItem('rashed_portfolio_data_corrupted_backup', stored); } catch (_) {}
    }
    this._data = JSON.parse(JSON.stringify(DEFAULT_DATA));
    this.save();
    return true;
  ...
  // Persists current data. Returns true on success, false on failure —
  // callers MUST check this so a failed save is never reported as a success.
  save() {
    if (!this._data) return false;
    try {
      this._data.lastUpdated = new Date().toISOString();
      localStorage.setItem('rashed_portfolio_data', JSON.stringify(this._data));
      this._lastError = null;
      return true;
    } catch (err) {
      console.error('Failed to save portfolio data:', err);
      if (err && (err.name === 'QuotaExceededError' || err.name === 'NS_ERROR_DOM_QUOTA_REACHED')) {
        this._lastError = 'Storage limit reached. Try a smaller image or shorter text, then save again.';
      } else {
        this._lastError = 'Could not save changes (' + (err && err.message ? err.message : 'unknown error') + ').';
      }
      return false;
    }
  },
  ...
  get() { return this._data; },
  set(data) { this._data = data; return this.save(); },
  update(path, value) {
    const parts = path.split('.');
    let cur = this._data;
    for (let i = 0; i < parts.length - 1; i++) cur = cur[parts[i]];
  ...
  // CRUD helpers — each returns true/false so the UI can tell success from failure
  addProject(p) { p.id = makeId(); this._data.projects.push(p); return this.save() ? p : null; },
  updateProject(id, data) {
    const idx = this._data.projects.findIndex(p => p.id === id);
    if (idx > -1) { this._data.projects[idx] = { ...this._data.projects[idx], ...data }; return this.save(); }
    return false;
  },
  deleteProject(id) { this._data.projects = this._data.projects.filter(p => p.id !== id); return this.save(); },

  addCategory(cat) { this._data.categories.push(cat); return this.save() ? cat : null; },
  deleteCategory(cat) { this._data.categories = this._data.categories.filter(c => c !== cat); return this.save(); },

  addTimeline(item) { item.id = item.id || makeId(); this._data.timeline.push(item); return this.save() ? item : null; },
  updateTimeline(id, data) {
    const idx = this._data.timeline.findIndex(t => t.id === id);
    if (idx > -1) { this._data.timeline[idx] = { ...this._data.timeline[idx], ...data }; return this.save(); }
    return false;
  },
  deleteTimeline(id) { this._data.timeline = this._data.timeline.filter(t => t.id !== id); return this.save(); },

  addSocial(s) { s.id = s.id || makeId(); this._data.social.push(s); return this.save() ? s : null; },
  updateSocial(id, data) {
    const idx = this._data.social.findIndex(s => s.id === id);
    if (idx > -1) { this._data.social[idx] = { ...this._data.social[idx], ...data }; return this.save(); }
    return false;
  },
  deleteSocial(id) { this._data.social = this._data.social.filter(s => s.id !== id); return this.save(); },

  addCertificate(c) { c.id = c.id || makeId(); this._data.certificates.push(c); return this.save() ? c : null; },
  updateCertificate(id, data) {
    const idx = this._data.certificates.findIndex(c => c.id === id);
    if (idx > -1) { this._data.certificates[idx] = { ...this._data.certificates[idx], ...data }; return this.save(); }
    return false;
  },
  deleteCertificate(id) { this._data.certificates = this._data.certificates.filter(c => c.id !== id); return this.save(); },

  updateResume(data) { this._data.resume = { ...this._data.resume, ...data }; return this.save(); },

  addExperience(exp) { exp.id = exp.id || makeId(); this._data.experience.push(exp); return this.save() ? exp : null; },
  updateExperience(id, data) {
    const idx = this._data.experience.findIndex(e => e.id === id);
...
// SAFE TEXT RENDERING
// ============================================================
// Escapes user-entered text before it is dropped into innerHTML, so
// characters like < > & " in titles/descriptions can never break the
// page layout or be misread as markup. Fields that intentionally allow
// simple formatting (the About paragraph) are NOT passed through this.
function escapeHTML(str) {
  if (str === null || str === undefined) return '';
  return String(str)
    .replace(/&/g, '&amp;')
...
// IMAGE COMPRESSION
// ============================================================
// Resizes an uploaded image down to a max bounding box (never enlarges
// smaller images) and re-encodes it via <canvas> before it is stored as
// base64 in localStorage. Keeps the same storage format (a data URL
// string in the same hidden input) — only the bytes going into that
// string get smaller. Returns a Promise<string> (the compressed data URL).
function compressImage(file, options) {
  options = options || {};
  const maxWidth = options.maxWidth || 800;
  const maxHeight = options.maxHeight || 800;
  const quality = options.quality || 0.85;
  // PNG uploads (icons/logos) keep PNG to preserve transparency; other
  // images are re-encoded as JPEG, which compresses photos far better.
  const outputType = options.outputType || (file.type === 'image/png' ? 'image/png' : 'image/jpeg');
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      const ctx = canvas.getContext('2d');
      if (outputType === 'image/jpeg') {
        // JPEG has no alpha channel — flatten onto white first so any
        // transparent source pixels don't turn black.
        ctx.fillStyle = '#ffffff';
        ctx.fillRect(0, 0, targetW, targetH);
      }
      ctx.drawImage(img, 0, 0, targetW, targetH);
...
// ============================================================
// A single, consistent, non-blocking way to tell the admin what happened.
// Replaces scattered alert() calls so failures are always visible and
// clearly worded instead of silently doing nothing.
function showNotice(message, type) {
  let el = document.getElementById('admin-notice');
  if (!el) {
    el = document.createElement('div');
...
// Call after any DataManager save-returning operation to surface success
// or failure consistently. Pass the boolean result of the DataManager call.
function notifySaveResult(ok, label) {
  if (ok) {
    showNotice(`${label} saved.`, 'success');
  } else {
    showNotice(`Failed to save ${label.toLowerCase()}: ${DataManager._lastError || 'unknown error'}`, 'error');
...
// DUPLICATE SUBMISSION GUARD
// ============================================================
// Prevents a double-click (or a slow second submit before the panel
// closes) from creating two records. Locks the form's submit button
// briefly; the button is always re-enabled even if something throws.
function guardDoubleSubmit(form) {
  if (form.dataset.submitting === 'true') return true; // already handling one — block this one
  form.dataset.submitting = 'true';
  const btn = form.querySelector('button[type="submit"]');
  if (btn) btn.disabled = true;
  setTimeout(() => {
...
function renderAll() {
  ...
  // Hero (textContent is used here, not innerHTML, so this is already safe from markup injection)
  document.getElementById('profileImage').src = data.hero.profileImage || DEFAULT_DATA.hero.profileImage;
  document.getElementById('heroFullName').textContent = data.hero.fullName || 'Rashedul Islam';
  document.getElementById('footerName').textContent = (data.hero.fullName || 'RASHEDUL ISLAM').toUpperCase();
  document.getElementById('hero-status').textContent = data.hero.status;
  ...
  // Social — circular icon buttons; uses the manually uploaded PNG logo when
  // present, otherwise falls back to the platform's lucide icon.
  document.getElementById('socialLinks').innerHTML = data.social.map(s => `
    <a href="${escapeHTML(s.url)}" target="_blank" rel="noopener noreferrer" class="social-link" title="${escapeHTML(s.label)}" aria-label="${escapeHTML(s.label)}">
      ${s.logo
        ? `<img src="${escapeHTML(s.logo)}" alt="${escapeHTML(s.label)}" class="social-logo-img" />`
  ...
  const resumeLabel = document.getElementById('resume-btn-label');
  if (data.resume && data.resume.fileData) {
    // Uploaded PDF — triggers an actual file download with a sensible filename.
    resumeBtn.href = data.resume.fileData;
    resumeBtn.setAttribute('download', data.resume.fileName || 'resume.pdf');
    resumeBtn.removeAttribute('target');
    resumeLabel.textContent = data.resume.label || 'Download Resume (PDF)';
...
// Picks the most recent project by its project date. Projects without a
// date can't be ranked by recency, so if none have one yet, the most
// recently added project (last in the array) is used as a sensible fallback.
function getLatestProject() {
  const data = DataManager.get();
  const projects = data.projects || [];
  if (!projects.length) return null;
...
// Formats a stored 'YYYY-MM-DD' project date as "Month YYYY" (e.g. August
// 2026) without going through the Date object, so there's no timezone-shift
// risk near month boundaries.
const PROJECT_MONTH_NAMES = ['January','February','March','April','May','June','July','August','September','October','November','December'];
function formatProjectDate(dateStr) {
  if (!dateStr) return '';
  const parts = dateStr.split('-');
...
function renderProjects(filter) {
  ...
  // Most recent project date first; projects without a date sink to the end.
  filtered = [...filtered].sort((a, b) => {
    if (!a.date && !b.date) return 0;
    if (!a.date) return 1;
    if (!b.date) return -1;
...
// Soft, modern donut palette. 'automation' and 'python' get fixed brand
// colors so the pills stay consistent across renders; anything else cycles
// through a pastel fallback palette.
const PIE_CATEGORY_STYLES = {
  automation: { solid: '#3b82f6', bg: 'rgba(59, 130, 246, 0.15)', text: '#3b82f6' },
  python:     { solid: '#f97316', bg: 'rgba(249, 115, 22, 0.15)', text: '#f97316' }
};
...
function renderPieChart() {
  ...
  const centerY = canvas.height / 2;
  const radius = Math.min(centerX, centerY) - 10;
  const holeRadius = radius * 0.62; // donut hole for the center "Total" label
  ...
  function drawPie(highlightIdx = -1) {
    ...
    // Punch a transparent hole in the center so the card background shows
    // through, turning the pie into a donut with a "Total" readout.
    ctx.save();
    ctx.globalCompositeOperation = 'destination-out';
    ctx.beginPath();
    ctx.arc(centerX, centerY, holeRadius, 0, Math.PI * 2);
...
// Builds one About-card editor block. Fields are found by data-field
// scoped to this block (not by a global index-based id), so adding or
// removing cards can never desync a field with the wrong card.
function buildAboutCardEditor(card) {
  card = card || {};
  const div = document.createElement('div');
  div.className = 'about-card-editor';
...
// Same fix as About cards: fields are scoped to their own row/group
// element via data-field, never via a global index-based id, so removing
// an item or group mid-list can never desync or silently drop siblings.
function buildSkillItemRow(item) {
  item = item || {};
  const row = document.createElement('div');
  row.className = 'form-row skill-item-row';
  ...
  row.querySelector('.skill-icon-file').addEventListener('change', async function(e) {
    ...
    const MAX_BYTES = 300 * 1024; // 300KB — plenty for a crisp icon, keeps storage light
    if (file.size > MAX_BYTES) {
      showNotice('Icon is too large (max 300KB). Please use a smaller PNG.', 'error');
      this.value = '';
      return;
    ...
    try {
      // Icons are small and square — resize down to a max box, keep PNG for transparency.
      const compressed = await compressImage(file, { maxWidth: 256, maxHeight: 256, outputType: 'image/png' });
      setPreview(compressed);
    } catch (err) {
      showNotice('Could not read that image file. Please try another.', 'error');
...
// Same scoped data-field pattern as About/Skills — no index-based ids,
// so adding/removing a stat can never desync a sibling's saved value.
function buildStatRow(stat) {
  stat = stat || {};
  const row = document.createElement('div');
  row.className = 'admin-list-item stat-row';
...
window.deleteProject = function(id) {
  ...
  if (!confirm(`Delete "${project.title}"?`)) return;
  const undoItem = { type: 'project', data: project };
  const ok = DataManager.deleteProject(id);
  renderAdminDashboard();
  renderAll();
  if (!ok) { showNotice(`Failed to delete: ${DataManager._lastError || 'unknown error'}`, 'error'); return; }
  showUndoToast(`"${project.title}" deleted`, () => {
    DataManager.addProject(undoItem.data);
    renderAdminDashboard();
    renderAll();
  });
};
...
window.deleteTimeline = function(id) {
  ...
  if (!confirm(`Delete "${item.title}"?`)) return;
  const undoItem = { type: 'timeline', data: item };
  const ok = DataManager.deleteTimeline(id);
  renderAdminDashboard();
  renderAll();
  if (!ok) { showNotice(`Failed to delete: ${DataManager._lastError || 'unknown error'}`, 'error'); return; }
  showUndoToast(`"${item.title}" removed`, () => {
    DataManager.addTimeline(undoItem.data);
    renderAdminDashboard();
    renderAll();
  });
};
...
window.deleteSocial = function(id) {
  ...
  if (!confirm(`Delete "${social.label}" link?`)) return;
  const undoItem = { type: 'social', data: social };
  const ok = DataManager.deleteSocial(id);
  renderAdminDashboard();
  renderAll();
  if (!ok) { showNotice(`Failed to delete: ${DataManager._lastError || 'unknown error'}`, 'error'); return; }
  showUndoToast(`"${social.label}" link deleted`, () => {
    DataManager.addSocial(undoItem.data);
    renderAdminDashboard();
    renderAll();
  });
};
...
window.deleteCertificate = function(id) {
  ...
  if (!confirm(`Delete certificate "${cert.title}"?`)) return;
  const undoItem = { type: 'certificate', data: cert };
  const ok = DataManager.deleteCertificate(id);
  renderAdminDashboard();
  renderAll();
  if (!ok) { showNotice(`Failed to delete: ${DataManager._lastError || 'unknown error'}`, 'error'); return; }
  showUndoToast(`"${cert.title}" deleted`, () => {
    DataManager.addCertificate(undoItem.data);
    renderAdminDashboard();
    renderAll();
  });
};
...
window.deleteExperience = function(id) {
  ...
  if (!confirm(`Delete experience at "${exp.company}"?`)) return;
  const undoItem = { type: 'experience', data: exp };
  const ok = DataManager.deleteExperience(id);
  renderAdminDashboard();
  renderAll();
  if (!ok) { showNotice(`Failed to delete: ${DataManager._lastError || 'unknown error'}`, 'error'); return; }
  showUndoToast(`"${exp.company}" removed`, () => {
    DataManager.addExperience(undoItem.data);
    renderAdminDashboard();
    renderAll();
  });
};
...
// Tags are picked from existing Skills (not free text), so a project's
// tags always line up with something in the Skills section.
function populateProjectTagSelect() {
  const data = DataManager.get();
  const select = document.getElementById('projectTagSelect');
  const skillNames = [...new Set(data.skills.flatMap(g => g.items.map(i => i.name)).filter(Boolean))];
...
// Thumbnail preview for the project form — shared by Add and Edit flows.
function setProjectThumbnailPreview(dataUrl) {
  const hidden = document.getElementById('projectThumbnail');
  const preview = document.getElementById('projectThumbnailPreview');
  hidden.value = dataUrl || '';
...
// Logo preview for the social link form — shared by Add and Edit flows.
function setSocialLogoPreview(dataUrl) {
  const hidden = document.getElementById('socialLogo');
  const preview = document.getElementById('socialLogoPreview');
  hidden.value = dataUrl || '';
...
function loginAdmin(password) {
  const storedHash = localStorage.getItem('rashed_portfolio_hash');
  if (!storedHash) {
    // First run safety net: shouldn't normally happen since init() seeds
    // a default hash, but if it's missing, set it from whatever was typed.
    localStorage.setItem('rashed_portfolio_hash', hashPassword(password));
    return true;
  }
  return hashPassword(password) === storedHash;
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Admin access — no visible link anywhere on the public site. The
  // panel is reached only via the private URL hash below (e.g.
  // yoursite.com/#admin), which still goes through the same password
  // gate and session state as before.
  function openAdminAccess() {
    if (localStorage.getItem('rashed_portfolio_hash') && adminLoggedIn) {
      document.getElementById('adminDashboard').classList.add('open');
      renderAdminDashboard();
  ...
  document.getElementById('heroProfileImageFile').addEventListener('change', async function(e) {
    ...
    // Images are stored as base64 inside localStorage (5-10MB total limit
    // per site), so keep uploads modest to leave room for the rest of the
    // portfolio content.
    ...
    try {
      // Resize down to a sensible profile-photo box before storing, so the
      // base64 that lands in localStorage is much smaller than the original.
      const compressed = await compressImage(file, { maxWidth: 600, maxHeight: 600 });
      document.getElementById('heroProfileImage').value = compressed;
      // Preview update handled on save
    } catch (err) {
      showNotice('Could not read that image file. Please try another.', 'error');
    }
  });
  ...
  document.getElementById('projectThumbnailFile').addEventListener('change', async function(e) {
    ...
    const MAX_BYTES = 1.5 * 1024 * 1024; // 1.5MB — stored as base64 in localStorage
    if (file.size > MAX_BYTES) {
      showNotice('Image is too large (max 1.5MB). Please choose a smaller file or compress it first.', 'error');
      this.value = '';
      return;
    ...
    try {
      // Thumbnails are wider than they are tall — cap on a larger box.
      const compressed = await compressImage(file, { maxWidth: 1000, maxHeight: 1000 });
      setProjectThumbnailPreview(compressed);
    } catch (err) {
      showNotice('Could not read that image file. Please try another.', 'error');
  ...
  document.getElementById('adminExpForm').addEventListener('submit', (e) => {
    ...
    const current = document.getElementById('expCurrent').checked;
    if (!company || !role || !description) { showNotice('Company, role, and description are required.', 'error'); return; }
    const ok = id ? DataManager.updateExperience(id, { company, role, location, start, end, description, current })
                  : !!DataManager.addExperience({ company, role, location, start, end, description, current });
    document.getElementById('adminExpFormPanel').style.display = 'none';
    renderAdminDashboard();
    renderAll();
  ...
  document.getElementById('adminAboutForm').addEventListener('submit', (e) => {
    e.preventDefault();
    const ok = DataManager.update('about.text', document.getElementById('aboutTextarea').value);
    renderAdminDashboard();
    renderAll();
    notifySaveResult(ok, 'About text');
  });

  // About Cards (save via button)
  document.getElementById('saveAboutCardsBtn').addEventListener('click', function() {
    const data = DataManager.get();
    const container = document.getElementById('adminAboutCards');
    const editors = container.querySelectorAll('.about-card-editor');
    ...
    if (skipped > 0) {
      ...
      notifySaveResult(ok, 'About cards');
    }
  });
  ...
  document.getElementById('adminCertForm').addEventListener('submit', (e) => {
    ...
    const url = document.getElementById('certUrl').value.trim();
    if (!title || !issuer || !description) { showNotice('Title, issuer, and description are required.', 'error'); return; }
    const ok = id ? DataManager.updateCertificate(id, { title, issuer, date, description, url })
                  : !!DataManager.addCertificate({ title, issuer, date, description, url });
    document.getElementById('adminCertFormPanel').style.display = 'none';
    renderAdminDashboard();
    renderAll();
    notifySaveResult(ok, 'Certificate');
  });
  ...
  document.getElementById('adminProjectForm').addEventListener('submit', (e) => {
    ...
    const date = document.getElementById('projectDate').value;
    if (!title || !description) { showNotice('Title and description are required.', 'error'); return; }
    const ok = id ? DataManager.updateProject(id, { title, description, image, tags, category, github, demo, date })
                  : !!DataManager.addProject({ title, description, image, tags, category, github, demo, date });
    document.getElementById('adminProjectFormPanel').style.display = 'none';
    renderAdminDashboard();
    renderAll();
    renderProjects(document.querySelector('.project-filters .filter-btn.active')?.dataset?.filter || 'all');
    notifySaveResult(ok, 'Project');
  });

  // Categories — managed inline from the Project form's Category dropdown
  document.getElementById('projectAddCategoryToggle').addEventListener('click', () => {
    const row = document.getElementById('inlineAddCategoryRow');
    const showing = row.style.display !== 'none';
    row.style.display = showing ? 'none' : 'flex';
  ...
  document.getElementById('confirmAddCategoryBtn').addEventListener('click', () => {
    ...
    const data = DataManager.get();
    if (data.categories.some(c => c.toLowerCase() === name.toLowerCase())) { showNotice('That category already exists.', 'error'); return; }
    const ok = !!DataManager.addCategory(name);
    if (ok) {
      populateCategorySelect();
      document.getElementById('projectCategory').value = name;
      renderProjectFilters();
  ...
  document.getElementById('adminTimelineForm').addEventListener('submit', (e) => {
    ...
    const current = document.getElementById('timelineCurrent').checked;
    if (!title || !description) { showNotice('Title and description are required.', 'error'); return; }
    const ok = id ? DataManager.updateTimeline(id, { title, description, current }) : !!DataManager.addTimeline({ title, description, current });
    document.getElementById('adminTimelineFormPanel').style.display = 'none';
    renderAdminDashboard();
    renderAll();
    notifySaveResult(ok, 'Timeline item');
  });
  ...
  document.getElementById('adminStatsForm').addEventListener('submit', (e) => {
    ...
    renderAll();
    if (skipped > 0) showNotice(`Saved, but ${skipped} stat(s) were skipped — both label and value are required.`, 'error');
    else notifySaveResult(ok, 'Stats');
  });
  ...
  // Manual PNG logo upload — no automatic platform detection or icon
  // assignment; the uploaded PNG is the only thing used as the icon.
  document.getElementById('socialLogoFile').addEventListener('change', async function(e) {
    const file = this.files[0];
    if (!file) return;
    if (file.type !== 'image/png') {
    ...
    const MAX_BYTES = 300 * 1024; // 300KB — plenty for a crisp logo icon
    if (file.size > MAX_BYTES) {
      showNotice('Logo is too large (max 300KB). Please use a smaller PNG.', 'error');
      this.value = '';
      return;
    ...
    try {
      // Logos are small and square — resize down to a max box, keep PNG for transparency.
      const compressed = await compressImage(file, { maxWidth: 256, maxHeight: 256, outputType: 'image/png' });
      setSocialLogoPreview(compressed);
    } catch (err) {
      showNotice('Could not read that image file. Please try another.', 'error');
  ...
  document.getElementById('adminSocialForm').addEventListener('submit', (e) => {
    ...
    const logo = document.getElementById('socialLogo').value.trim();
    if (!url) { showNotice('URL is required.', 'error'); return; }
    const ok = id ? DataManager.updateSocial(id, { platform, label, url, logo })
                  : !!DataManager.addSocial({ platform, label, url, logo });
    document.getElementById('adminSocialFormPanel').style.display = 'none';
    renderAdminDashboard();
    renderAll();
    notifySaveResult(ok, 'Social link');
  });

  // Contact
  document.getElementById('adminContactForm').addEventListener('submit', (e) => {
    e.preventDefault();
    const ok = DataManager.update('contact.text', document.getElementById('contactTextarea').value);
    renderAdminDashboard();
    renderAll();
  ...
  document.getElementById('resumeFileInput').addEventListener('change', function(e) {
    ...
    const MAX_BYTES = 4 * 1024 * 1024; // 4MB — leaves room for the rest of the site's data
    if (file.size > MAX_BYTES) {
      showNotice('PDF is too large (max 4MB). Try compressing it or link an external URL instead.', 'error');
      this.value = '';
      return;
  ...
  document.getElementById('adminResumeForm').addEventListener('submit', (e) => {
    ...
    const fileData = document.getElementById('resumeFileData').value;
    const fileName = document.getElementById('resumeFileName').value;
    const ok = DataManager.updateResume({ url, label, fileData, fileName });
    renderAdminDashboard();
    renderAll();
  ...
  // Generate filter buttons from categories for projects
  renderProjectFilters();

});

// Rebuilds the project filter buttons from the current category list
// (keeps the "All" button, replaces the rest). Called on load and
// whenever a category is added from the Project form.
function renderProjectFilters() {
  const filterContainer = document.querySelector('.project-filters');
  const categories = DataManager.get().categories || [];
  filterContainer.querySelectorAll('.filter-btn:not([data-filter="all"])').forEach(b => b.remove());
